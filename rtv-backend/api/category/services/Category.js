...
module.exports = {
  ...
  fetch: params => {
    ...
    return Category.findOne(
      _.pick(params, _.keys(Category.schema.paths))
    ).populate(populate);
  },
  ...
  count: params => {
    ...
    return Category.count().where(filters.where);
  },
  ...
  add: async values => {
    ...
    const relations = _.pick(
      values,
      Category.associations.map(ast => ast.alias)
    );
  ...
  edit: async (params, values) => {
    ...
    return Category.updateRelations(
  ...
  remove: async params => {
    ...
    const data = await Category.findOneAndRemove(params, {}).populate(populate);
